...
class Pyr():
    def __init__(self, NDim=2, basis='PhasePixel', RegMat='D1', npup=33, npad=1024,
                 pupil_geom='circle', lam=0.85, d=7.2e3, D=7.992e6,
    ...
    # RegParam - regularization parameter, can be None
    ...
    def LeastSqSol(self, y, wt, c0, RegParam=None, ZeroPhaseMean=True):
        assert (len(y) == self.nd) and (len(c0) == self.nc)
        if wt is not None:
            assert len(wt) == len(y)
        if RegParam is not None:  # Cost sets value of self.RegScale
            (cost, dcost) = self.Cost(c0, y, wt, RegParam=1, ZeroPhaseMean=True, AmpCon=False)
    ...
    def Cost(self, c, y, wt=None, RegParam=None, ZeroPhaseMean=True, AmpCon=False):
        ...
        if wt is None:
            ...
            assert len(wt) == len(y)
            wt0 = wt
        if (self.RegHess is None) or (self.RegScale is None):
            if self.basis_type == 'ReImPixel':
...
#                dcost[k1] += 4.*const*c[k2]*(c[k1]*c[k1] + c[k2]*c[k2] - AmpVal)
    ...
    def CostHessian(self, c, y, wt=None, RegParam=None, ZeroPhaseMean=True, AmpCon=False):
        assert len(y) == self.nd
        if wt is None:
            wt0 = np.ones(y.shape)
        else:
        ...
        if RegParam is not None:
            if self.RegHess is None:  # set it up
                self.Cost(c, y, wt, RegParam=RegParam, AmpVal=1., AmpFac=.1, ZeroPhaseMean=ZeroPhaseMean)
            hes += self.RegHess
        if not AmpCon:
            return(hes)
...
#                hes[k2, k1] += hes[k1, k2]
    ...
    def PupilImageIntensity(self, paramvec, g=None, slopes=True):
        ...
        for k1 in range(sz):
            ...
            se = I[p2d[3]]
            if slopes:
                S.append(nw + sw - ne - se)
                S.append(nw + ne - sw - se)
            else:
                S.append(nw)
                S.append(sw)
                S.append(ne)
                S.append(se)
            nw = dI[p2d[0][0], p2d[0][1], :]
            sw = dI[p2d[1][0], p2d[1][1], :]
            ne = dI[p2d[2][0], p2d[2][1], :]
            se = dI[p2d[3][0], p2d[3][1], :]
            if slopes:
                dS.append(nw + sw - ne - se)
                dS.append(nw + ne - sw - se)
            else:
                dS.append(nw)
                dS.append(sw)
                dS.append(ne)
        ...
        if slopes:
            S = S.reshape(2*sz)
            dS = dS.reshape(2*sz, self.nc)
        else:
            S = S.reshape(4*sz)
            dS = dS.reshape(4*sz, self.nc)
        return(S, dS)
